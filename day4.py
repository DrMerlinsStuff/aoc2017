passphrases='''sayndz zfxlkl attjtww cti sokkmty brx fhh suelqbp
...
totalW=0
totalA=0
for pp in passphrases.split("\n"):
  words={}
  anagrams={}
  successW=True
  successA=True
  for word in pp.split(" "):    
    mysort=''.join(sorted(word))
    if word in words.keys():
      print word + " is duplicated in " + pp
      successW=False
      #break
    else:
	  words[word]=1
  
    if mysort in anagrams.keys():
      print word + " is anagrammed in " + pp
      successA=False
      #break
    else:
	  anagrams[mysort]=1
