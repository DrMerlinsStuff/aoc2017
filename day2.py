#import numpy
#matrix=numpy.empty((4,3))
from __future__ import division

array="179	2358	5197	867	163	4418	3135	5049	187	166	4682	5080	5541	172	4294	1397;\
...
for i in range(len(matrix)):
    matrix[i]=[int(x) for x in array.split(';')[i].split('\t')]
...
def star_one():
  ...
  for row in matrix:
    row.sort(key=int)
    total+=int(row[len(row)-1]) - int(row[0])
  print total
...
def star_two():
  ...
  for row in matrix:
    for col in range(len(row)-1):      
      for other in range(col+1,len(row)):
        #print ("operands",col,other)
        left=row[other]/row[col]
        right=row[col]/row[other]
